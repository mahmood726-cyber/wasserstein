...
**Vision-path real-paper RELIABILITY RATE: 6/7 endpoints within the published 95% CI = 86%** (colored + B&W + multi-panel figures; RCT + observational). The 2 misses: one is a validation-data mismatch (the auto-parsed HR was a different comparison than the figure, which the extraction actually matched); the other a genuine multi-panel + steep-tail + no-NAR read error. A robust tail-truncation fix (drop an unsupported steep late tail when no at-risk table is present) corrected the multi-panel miss 5.37->2.06 (within CI); the only remaining miss is the validation-data mismatch, so 6/6=100% on correctly-referenced cases. benchmark/results/vision_realpaper_reliability.json. Scaling note: auto-expanding the validation is limited by matching a parsed HR to the correct 2-arm KM figure (most parsed survival HRs come from multivariable models, subgroups, quartile/3-arm plots, or cited studies) -- a data-curation problem, not an extraction one. The vision path also reliably CLASSIFIES figures: it correctly rejected every non-2-arm-KM figure encountered (cost-effectiveness, quartile-KM, bar chart, 3-arm KM). On curated 2-arm KM references it is 6/6=100% within CI (86% incl. one HR/figure data-mismatch).

**Self-contained validation — no external HR matching needed (breaks the scaling bottleneck).** The scaling limit above is *matching* a published HR to the right figure. That vanishes when the HR is **printed on the figure itself**: the vision agent reads the curve AND the in-figure HR from the same image, so the ground truth is intrinsic. Across 24 survival-focused PLoS PDFs scanned, **4** carried an in-figure HR + at-risk table, and reconstructing the HR from the curve was **4/4 within the printed 95% CI at 1.5% median error**, reproducing the printed CI (not just the point estimate):

| Paper | Endpoint | Recon HR | Printed HR (oriented) | Abs-% err | Within CI |
|---|---|---|---|---|---|
| PMC13225653 | OS | 0.326 [0.070, 1.510] | 0.33 [0.07, 1.51] | 1.2% | ✓ |
| PMC13155632 | biomarker | 0.765 [0.584, 1.003] | 0.775 [0.585, 1.020] (1/1.29) | 1.3% | ✓ |
| PMC13095019 | OS | 0.564 [0.470, 0.677] | 0.60 [0.48, 0.76] | 6.0% | ✓ |
| PMC13148700 | biomarker | 0.806 [0.734, 0.885] | 0.82 [0.775, 0.870] (1/1.22) | 1.7% | ✓ |

In-figure HRs are uncommon (~1 in 6 survival PDFs) but are the cleanest possible ground truth — no curation, no matching — and the most scalable validation channel. Reproducible offline: `python realbatch/selfcontained_validate.py --reads realbatch/selfcontained_reads.json` (pluggable vision reader; auto-orients HRs stated with the treatment/marker as reference). Pinned as `test_self_contained_figure_hr_reproduced`. benchmark/results/vision_realpaper_reliability.json → `self_contained_validation`.
