...
**Self-contained validation — no external HR matching needed (breaks the scaling bottleneck).** The scaling limit above is *matching* a published HR to the right figure. That vanishes when the HR is **printed on the figure itself**: the vision agent reads the curve AND the in-figure HR from the same image, so the ground truth is intrinsic. On the two real PLoS figures found carrying an in-figure HR + at-risk table, reconstructing the HR from the curve **reproduced the printed HR to 1.3% median error and reproduced the printed 95% CI to the second decimal**: PMC13225653 OS recon 0.326 [0.070, 1.510] vs printed 0.33 [0.07, 1.51]; PMC13155632 recon 0.765 [0.584, 1.003] vs printed 0.775 [0.585, 1.020] (1/1.29). In-figure HRs are uncommon (~2 of 14 survival PDFs) but are the cleanest possible ground truth — no curation, no matching. Pinned as a regression test (`test_self_contained_figure_hr_reproduced`). benchmark/results/vision_realpaper_reliability.json → `self_contained_validation`.
